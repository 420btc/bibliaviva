...
const InteractiveShader = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      if (iResolutionLocation) gl.uniform2f(iResolutionLocation, gl.canvas.width, gl.canvas.height);
    };
    ...
    const renderLoop = () => {
      ...
      if (iMouseLocation) gl.uniform2f(iMouseLocation, mousePos.current.x * canvas.width, (1.0 - mousePos.current.y) * canvas.height);
      if (uFlowSpeedLocation) gl.uniform1f(uFlowSpeedLocation, flowSpeed);
